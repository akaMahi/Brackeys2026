...
#       ...your own movement, ending with a `var pre_move_speed := velocity.length()`
...
@export var knockback_rotation_kick: float = 0.6 # radians of "spin" per 1000 units/sec of impact speed
...
func _apply_knockback(collision_normal: Vector2, impact_speed: float) -> void:
	if impact_speed < min_knockback_speed:
		return
	velocity += collision_normal * impact_speed * knockback_strength

	var spin_direction: float = sign(collision_normal.x) if collision_normal.x != 0.0 else 1.0
	rotation += spin_direction * knockback_rotation_kick * (impact_speed / 1000.0)
